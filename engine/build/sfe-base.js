(function () {
	...
	var Vue$1 = unwrapExports(vue2_5_17);
	...
	class ConfigLoader {
	        ...
	        return Promise.all(this.awaitConfigs)
	            .then(results => {
	                if (results[0] === true &&
	                    results[1] === true &&
	                    results[2] === true) {
	                    console.log(LAYER$2 + 'USERDATA CHECKED');
	                    return this.userData;
	                } else {
	                    console.log(LAYER$2 + 'USERDATA LOADED');
	                    return this.userData;
	                }
	            }); 
	    }
	    ...
	    async checkAppConfig() {
	        ...
	        if (SFE.isEmpty(this.userData.APPID) ||
	            SFE.isEmpty(this.userData.APPMODE) ||
	            SFE.isEmpty(this.userData.INFO)) {
	            const url = this.userData.paths.appPath+'configs/appconfig.json5';
	            appConfig = await SFE.getJsonFile(url);
	    ...
	    async checkAppLibraries() {
	        ...
	        // 라이브러리 컨피그 검사
	        // 존재하지 않을 경우 파일로드해서 선언
	        if (SFE.isEmpty(this.userData.libs)) {
	            const url = this.userData.paths.appPath+'configs/applibraries.json5';
	            appLibraries = await SFE.getJsonFile(url);
	...
	class LibraryLoader {
	    ...
	    async load() {
	        ...
	        .then(function () {
	            ...
	            console.log(LAYER$3 + 'LIBRARY LOADED');
	        });
	    ...
	    async checkModule(script) {
	        ...
	        if (check(script.id) === undefined) {
	            ...
	            if (script.src !== undefined) ; else if (script.roader !== undefined) ;
	        }
	...
	class AppLoader {
	    ...
	    startApp(startScript = null) {
	        ...
	        console.log(LAYER$4 + 'START');
	    }

	}

	// SFE Dependancy
	window.JSON5 = json52_1_0_min;
	window.Cookies = js_cookie2_2_0_min;
	window.Vue = Vue$1;
