...
// FIXED: 모듈 익스포트 처리
var THREE = {}
import * as Three from '../three-r97.module.js';
Object.assign(THREE, Three);
export { THREE }

import { THREE as TGALoader } from './TGALoader.module.js';
...
THREE.FBXLoader.Custom = class extends THREE.FBXLoader {
    ...
    constructor(Assets) {
		...
		this.manager.onLoad = this.Assets.onLoad;
...
THREE.FBXTreeParser.Custom = class extends THREE.FBXTreeParser {
	// FIXED: Assets 인수
    constructor(model, Assets) {
		...
		this.tgaLoader.setPath(this.textureLoader.path);
		THREE.Loader.Handlers.add(/\.tga$/i, this.tgaLoader);
	}
	...
	parseEmbedImages() {
		...
		for (var nodeID in videoNodes) {
			...
			// FIXED: 이미지 선택 조건 간소화.
			if (content !== undefined && ((content instanceof ArrayBuffer) && (content.byteLength > 0)) === false) {
				continue;
			}

			// FIXED: parseImage() 메서드 간소화.
			...
			if (content !== undefined) {
				embedList.push(fileName);
				images[fileName].embedded = this.model.name;
	...
	parseEmbedTextures(images) {
		var textureMap = new Map();
		if (fbxTree.Objects.Texture === undefined) {
			return textureMap;
		}
		var Assets = this.Assets;

		for (var id in images) {
			images[id].onStart = function() {
				Assets.Loader.queue.push(
					new Promise((resolve) => {
			...
			images[id].onProgress = function() {
				Assets.Loader.changeFileProgress(this);
			}
			images[id].load();
		}	
		var textureNodes = fbxTree.Objects.Texture;
		for (var nodeID in textureNodes) {
			...
			var file = textureNode.RelativeFilename || textureNode.Filename;
			var fileName = getFileStrFromDir(file);

			var texture = images[fileName].data;
			texture.ID = textureNode.id;
			texture.name = textureNode.attrName;
	...
	parseParameters(materialNode, textureMap, ID) {
		...
		if (materialNode.vertexColors) {
			...
			parameters.vertexColors = THREE.NoColors;
		}

		// side. 어느쪽 면이 랜더링될지를 결정. 
		// THREE.FrontSide = 0 (기본값), THREE.BackSide = 1, THREE.DoubleSide = 2
		// API: https://threejs.org/docs/#api/materials/Material.side
		// 투명재질이거나 오파시티가 들어갈 경우 더블사이드로 지정. 
		if (materialNode.side) {
			parameters.side = materialNode.side.value;
		} else {
			parameters.side = THREE.FrontSide;
		}

		// shadowSide. 어느쪽 면이 그림자를 받아들일지를 결정.
		// THREE.FrontSide = 0, THREE.BackSide = 1, THREE.DoubleSide = 2 (기본값은 side 값과 동일하게 지정됨)
		// API: https://threejs.org/docs/#api/materials/Material.shadowSide
		// side 가 DoubleSide 일 경우 그림자 랜더링에 문제가 생기므로 light.shadow.bias 값을 -0.001 로 설정할 필요 있음
		// reference: https://github.com/mrdoob/three.js/issues/8692
		// reference: https://stackoverflow.com/questions/32545283/shadow-artifacts-on-double-sided-plane/32546348#32546348
		if (materialNode.shadowSide) {
			parameters.shadowSide = materialNode.shadowSide.value;
		} else if (parameters.side) {
			parameters.shadowSide = parameters.side;
		} else {
			parameters.shadowSide = THREE.FrontSide;
		}
		...
		// transparent. 투명 처리.
		// API: https://threejs.org/docs/#api/materials/Material.transparent
		if (materialNode.transparent) {
			parameters.transparent = materialNode.transparent;
		}

		// Opacity. 투명도
		// API: https://threejs.org/docs/#api/materials/Material.opacity
		if (materialNode.Opacity) {
			parameters.opacity = parseFloat(materialNode.Opacity.value);
			if (parameters.opacity < 1.0) {
				parameters.transparent = true;
		...
		// premultipliedAlpha.
		// API: https://threejs.org/docs/#api/materials/Material.premultipliedAlpha
		if (materialNode.premultipliedAlpha) {
			parameters.premultipliedAlpha = materialNode.premultipliedAlpha;
		}

		// blending. 오파시티 블렌딩 처리.
		...
		// 커스텀 블렌딩 처리
		// blendEquation :
		// THREE.AddEquation = 100 (기본값), THREE.SubtractEquation = 101, THREE.ReverseSubtractEquation = 102, 
		// THREE.MinEquation =3, THREE.MaxEquation = 4
		// blendSrc , blendDst :
		// THREE.ZeroFactor = 200, THREE.OneFactor = 201, THREE.SrcColorFactor = 202, THREE.OneMinusSrcColorFactor = 203,
		// THREE.SrcAlphaFactor = 204 (blendSrc 기본값), THREE.OneMinusSrcAlphaFactor = 205 (blendDst 기본값), THREE.DstAlphaFactor = 206, THREE.OneMinusDstAlphaFactor = 207,
		// THREE.DstColorFactor = 208, THREE.OneMinusDstColorFactor = 209, THREE.SrcAlphaSaturateFactor = 210
		// API: https://threejs.org/docs/#api/materials/Material.blendEquation
		// API: https://threejs.org/docs/#api/materials/Material.blendSrc
		// API: https://threejs.org/docs/#api/materials/Material.blendDst
		// API: https://threejs.org/docs/#api/constants/CustomBlendingEquations
		// sample: https://threejs.org/examples/#webgl_materials_blending_custom
		if (parameters.blending == THREE.CustomBlending) {

			if (materialNode.blendEquation) {
				parameters.blendEquation = 100 + materialNode.blendEquation.value;
			} else {
				parameters.blendEquation = THREE.AddEquation;
			}

			if (materialNode.blendSrc) {
				parameters.blendSrc = 200 + materialNode.blendSrc.value;
			} else {
				parameters.blendSrc = THREE.SrcAlphaFactor;
			}

			if (materialNode.blendDst) {
				parameters.blendDst = 200 + materialNode.blendDst.value;
			} else {
				parameters.blendDst = THREE.OneMinusSrcAlphaFactor;
			}

		}

		// alphaTest. 오파시티 블랜딩을 클립 처리한다.
		// API: https://threejs.org/docs/#api/materials/Material.alphaTest
		if (materialNode.alphaTest) {
			parameters.alphaTest = materialNode.alphaTest.value;
			if (!materialNode.blending) {
				parameters.blending = THREE.NormalBlending;
			}
		}

		// emissive. 3ds max 의 self-illuminaton 컬러 및 값.
		// 컬러일 경우 해당 컬러가 디퓨즈컬러에 플러스되어서 밝게 표시되고 EmissiveFactor 는 null 처리된다.
		if (materialNode.Emissive) {
			parameters.emissive = new THREE.Color().fromArray(materialNode.Emissive.value);
		}

		// EmissiveColor 및 EmissiveFactor. 3ds max 의 self-illuminaton 값.
		// 컬러일 경우 0,0,0 부터 1,1,1 까지. 1일 경우 백색으로 표시된다. 기본값은 0,0,0 
		// 값일 경우 0~1 까지. 1일 경우 완전 플랫한 쉐이딩으로 표시된다. 기본값은 1. (맥스에서는 기본값 0으로 설정되어 넘어옴)
		// API: https://threejs.org/docs/#api/materials/MeshPhongMaterial.emissiveIntensity
		if (materialNode.EmissiveColor && materialNode.EmissiveColor.type === 'Color') {
			// The blender exporter exports emissive color here instead of in materialNode.Emissive
			parameters.emissive = new THREE.Color().fromArray(materialNode.EmissiveColor.value);
		} else if (materialNode.EmissiveFactor) {
			parameters.emissiveIntensity = parseFloat(materialNode.EmissiveFactor.value);
		} else {
			parameters.emissiveIntensity = 1;
		}

		// reflectivity = ReflectionFactor. 반사도. 3ds max 의 reflection 슬롯의 amount 값.
		// API: https://threejs.org/docs/#api/materials/MeshPhongMaterial.reflectivity
		if (materialNode.ReflectionFactor) {
			parameters.reflectivity = materialNode.ReflectionFactor.value;
		}

		// combine. 반사맵이 표면에 씌워지는 방식을 결정.
		// THREE.Multiply = 0, THREE.MixOperation = 1, THREE.AddOperation = 2 (기본값)
		// API: https://threejs.org/docs/#api/materials/MeshPhongMaterial.combine
		if (materialNode.combine) {
			parameters.combine = materialNode.combine.value;
		} else {
			// parameters.combine = THREE.Multiply; // 원래 기본값
			parameters.combine = THREE.AddOperation;
		}

		// displacementScale = DisplacementFactor. 디스플레이스먼트 맵의 돌출도. 3ds max 의 displacement 슬롯의 amount 값.
		// API: https://threejs.org/docs/#api/materials/MeshPhongMaterial.displacementScale
		if (materialNode.DisplacementFactor) {
			parameters.displacementScale = materialNode.DisplacementFactor.value;
		}
		...
		// Emissive 재처리. 3ds Max 의 self-illuminaton 값이 그대로 반영되도록 한다.
		if (parameters.emissiveIntensity > 0) {
			// 디퓨즈맵이 존재하고 에미시브맵이 없을 경우
			if (parameters.map && !parameters.emissiveMap) {
				parameters.emissiveMap = parameters.map;
				if (JSON.stringify(materialNode.Emissive.value) == JSON.stringify([0, 0, 0])) {
					var intensity = parameters.emissiveIntensity;
					parameters.emissive = new THREE.Color().fromArray([
						intensity, intensity, intensity
					]);
				}
			// 디퓨즈 컬러가 컬러값만 존재할 경우
			} else if (!parameters.map && !parameters.emissiveMap &&
				JSON.stringify(materialNode.Emissive.value) == JSON.stringify([0, 0, 0])) {
